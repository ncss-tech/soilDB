...
| String concat | `col1 + col2` | `col1 || col2` | Different operators |
...
confirmation of filtering logic. You can also include "MPM" and "HPM" if you are
interested in excluding even more decomposed dry organic materials:
...
1. **Record Limit**: Queries that return more than **100,000 records** may be rejected or truncated
...
`makeChunks()` is a helper function in the soilDB package. It takes a vector of
values as input, and assigns each value a group (chunk) number. You can then
...
When you need data for many map units, components, or other entities, a single
query can exceed SDA's limits.
...
Suppose you have 5000 mukeys to query:

```{r chunking-example}
large_mukey_list <- 461994:466994
...
for (i in unique(chunks)) {
  # Subset large list to extract just values in chunk `i`
  chunk_keys <- large_mukey_list[chunks == i]
 
  # construct query 
  q <- sprintf(
    "SELECT mukey, musym, muname FROM mapunit WHERE mukey IN (%s)",
    paste(chunk_keys, collapse = ",")
  )
  
  results_list[[i]] <- SDA_query(q)
  
  cat(sprintf("Chunk %d of %d\n", i, length(unique(chunks))))
}
...
Here we query soil properties for 10 map units in chunks:

```{r chunked-property-example}
test_mukeys <- c(461994, 461995, 462205, 462206, 462207, 
                 463263, 463264, 464571, 464572, 465186)

# Chunk into groups of 5
chunks <- soilDB::makeChunks(test_mukeys, 5)

results_list <- list()
for (i in unique(chunks)) {
  chunk_keys <- test_mukeys[chunks == i]
  
  # Build IN clause
  in_clause <- paste(chunk_keys, collapse = ",")
  ...
  results_list[[i]] <- SDA_query(q)
  cat(sprintf("Chunk %d of %d completed\n", i, length(unique(chunks))))
}
...
Also, you can do the inverse (mukey to geometry):

```{r sda-helper-function-geometry}
# 2. Get Geometry from Map Unit Key (mukey)
