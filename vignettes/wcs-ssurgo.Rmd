---
title: "Thematic Maps of SSURGO Data"
output: 
  html_document:
    mathjax: null
    jquery: null
    ...
    toc_float:
      ...
      smooth_scroll: false
vignette: >
  %\VignetteIndexEntry{Thematic Maps of SSURGO Data via Soil Data Access and SoilWeb Web Coverage Services}
...
A web coverage service (WCS) is provided for the gSSURGO and gNATGSO [map unit key grids](https://nrcs.app.box.com/v/soils/folder/180456387335). These grids are a raster representation of the gSSURGO and gNATSGO map unit keys (`mukey`) for the conterminous United States at a resolution of 30m.  The GeoTIFF format is used to ensure maximum compatibility. Cell values are map unit keys, encoded as unsigned 32-bit integers. The standard spatial reference for these grids is Albers Equal Area Conic (NAD83) coordinate reference system (`"EPSG:5070"`). The grids are 'LZW' compressed and internally tiled for efficient random access. The grid topology and cell values are identical to the rasters contained within the gSSURGO and gNATSGO File Geodatabases (FGDB).

The files backing each WCS are re-created annually as part of the SSURGO refresh cycle. The gSSURGO and gNATSGO grids are updated annually, and typically in-sync with the live version of the data hosted by SDA (updated Oct 1st) by early November. Map unit keys can change over time, especially in soil survey areas that were updated during the last fiscal year.

In addition to the standard CONUS gSSURGO and gNATSGO grids, the following web coverage services are also provided:

 - Raster Soil Survey (RSS) products at 10m resolution.

 - STATSGO2 (2016) at 300m resolution.
...
Get the latest version of `soilDB` and `terra`, along with all packages specified below.

```{r eval = FALSE}
install.packages(c('soilDB', 'terra'))
```

Consider also installing the latest development versions from GitHub/r-universe:
...
Here are some basic usage examples for the various coverage services that can be accessed with `mukey.wcs()`.
...
Inspect the result by overlay SSURGO polygons on the 30m map unit key grid.
...
# merge data values in to raster attribute table, joining on "mukey" column
levels(mu2) <- tab

# convert grid + RAT -> stack of propery grids
aws <- catalyze(mu2)

# simple plot, set a common range [0, 20] for both layers
plot(
  aws,
  axes = FALSE,
  main = c(
...
A widely-used property which is calculated as a standard part of SSURGO soil map unit components is the "steel corrosion potential".
...
tab$corsteel <- factor(tab$corsteel, levels = c("Low", "Moderate", "High"), ordered = TRUE)
...
Another example is thematic mapping of the "simplified component parent material group".
...
We can also inspect the "Hydric Rating".
...
First, set up a new AOI for the following examples:
...
plot(
  c(ssc, texture.class),
  main = 'Sand, Silt, Clay (RV) & Texture Class\nDominant Component\n25-50cm'
)
```
