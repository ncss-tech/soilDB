...
This is a draft guide that identifies the source tables and columns for each column in the `fetchNASIS(from='pedons')` result. Many data elements have a 1:1 relationship with a table and column in NASIS, these are described in order of alphabetical order of their parent NASIS table. 
...
calcdef <- structure(list(fetchNASIS_Column = c("hzID", "d_r", "d_g", "d_b", 
...
"albic.materials"), ColumnDescription = c("Internal horizon row ID (unique) calculated in aqp SoilProfileCollection objects", 
...
"Rock fragment volume (%) in 2 - 5 mm size range", "Rock fragment volume (%) in 2 - 76 mm size range (includes fine_gravel)", 
"Rock fragment volume (%) in the 76 - 250 mm size range", "Rock fragment volume (%) in the 250 - 600 mm size range", 
...
"Pararock fragment volume (%) in 2 - 76 mm size range (includes parafine_gravel)", 
"Pararock fragment volume (%) in the 76 - 250 mm size range", 
...
"Artifact volume (%) in 2 - 5 mm size range", "Artifact volume (%) in 2 - 76 mm size range (includes fine_gravel)", 
"Artifact volume (%) in the 76 - 250 mm size range", "Artifact volume (%) in the 250 - 600 mm size range", 
...
"Deprecated alias of surface_fine_gravel", "Surface rock fragment volume (%) in 2 - 76 mm size range (includes fine_gravel)", 
"Surface rock fragment volume (%) in the 76 - 250 mm size range", 
...
"Surface pararock fragment volume (%) in 2 - 76 mm size range (includes parafine_gravel)", 
"Surface pararock fragment volume (%) in the 76 - 250 mm size range", 
...
"Presence/absence. Calculated from Pedon Diagnostic Horizon Table with `soilDB:::.diagHzLongtoWide()`", 
"Presence/absence. Calculated from Pedon Diagnostic Horizon Table with `soilDB:::.diagHzLongtoWide()`"
), RelatedFunction = c("`get_hz_data_from_NASIS_db()`", "`get_colors_from_NASIS_db()`", 
...
"`get_extended_data_from_NASIS_db()`", "`get_extended_data_from_NASIS_db()`", 
"`get_extended_data_from_NASIS_db()`", "`get_extended_data_from_NASIS_db()`"
), Deprecated = c(FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, 
...
FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, 
FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, FALSE, 
FALSE, FALSE, FALSE, FALSE)), row.names = c(NA, -92L), class = "data.frame")
...
newalias <- merge(data.table::data.table(res[,1:2]), ressub, all.x = TRUE, sort = FALSE)$fetchNASIS_Column
...
makeTable <- function(d) {
  ...
  colnames(d)[3] <- "Column Description"
  colnames(d)[4] <- "`fetchNASIS()` Column"
  d$`Physical Name` <- paste0(d$TablePhysicalName, ".", d$ColumnPhysicalName)
  cat(knitr::kable(d[c(4:6, 3)], row.names = FALSE), sep = "\n")
  cat("\n")
  cat("\n")
}
