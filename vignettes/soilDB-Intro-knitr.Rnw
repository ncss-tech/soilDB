...
% \SweaveOpts{fig.path=figure/graphics-, cache.path=cache/graphics-, fig.align=center, dev=tikz, external=TRUE, fig.width=5, fig.height=5, fig.show=hold, cache=TRUE, par=TRUE}
...
control of plot sizes, the tikz device, animations and other types
of plots such as \textbf{rgl} or GGobi plots.
\end{abstract}
Before reading this specific manual%
\footnote{source code at \url{https://github.com/yihui/knitr/blob/master/inst/examples/knitr-graphics.Rnw}%
...
some multi-byte characters (say hello in Chinese):
	...
	<<cairo-pdf-dev,dev=cairo_pdf,fig.width=8,fig.height=2,out.width=.8\linewidth,warning=FALSE,tidy=FALSE,echo=FALSE>>=
...
for the \texttt{white} option like this:
	
	<<white-background,eval=FALSE>>=
	knit_hooks$set(white = function(before, options, envir) {if (before) par(bg = 'white')})
@

Then set \texttt{white=TRUE} in the global option (e.g. \texttt{\textbackslash{}SweaveOpts\{white
																																														= TRUE\}}). Note the hook name \texttt{white} is arbitrary; see the
\href{http://yihui.github.com/knitr/hooks}{hooks} page in the website.
...
\begin{figure}
<<two-high,fig.width=3,fig.height=2,out.width=.49\linewidth>>=
	plot(cars)
...
Together with \texttt{fig.show=asis}, we can show the process of plotting
step by step like Figure \ref{fig:high-with-low}.

\begin{figure}
<<high-with-low,fig.width=3,fig.height=2,out.width=.5\linewidth,fig.keep=all,fig.show=asis>>=
	plot(cars)
...
(only the last plot is kept):
	...
	\begin{figure}
<<fig-last,fig.width=5.2,fig.height=3,out.width=.7\linewidth,fig.keep=last>>=
...
them as an animation with the option \texttt{fig.show=animate}. Figure
\ref{fig:clock-animation} shows a simple clock animation; you may
compare the code to Figure \ref{fig:high-with-low} to understand
...
option; this document uses \texttt{fig.align=center} as a global option,
...
\begin{figure}
<<math-formula-tt,fig.width=6,fig.height=2,out.width=\linewidth,echo=T>>=
...
Now we compare two plots below. The first one is not cropped:
	...
	\begin{figure}
<<pdf-nocrop,fig.width=3.2,fig.height=3.2,out.width=.4\linewidth>>=
	test_crop=function(){
...
which will call the cropping hook:
	...
	\begin{figure}
<<pdf-crop,echo=FALSE,fig.width=3.2,fig.height=3.2,out.width=.4\linewidth,crop=TRUE,dependson=pdf-nocrop>>=
	test_crop()
@
