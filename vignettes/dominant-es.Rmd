---
title: "Mapping Dominant Ecological Sites with soilDB"
author: "Andrew G. Brown"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Mapping Dominant Ecological Sites with soilDB}
...
Here is an example using STATSGO. We use `SDA_spatialQuery()`
...
- `SDA_spatialQuery()` is ideal for spatial queries where you have a specific area of interest.
...
If working with large extents, it is generally better to be use a local data source. You can download SSURGO data from Web Soil Survey using `downloadSSURGO()`. Then, you can create GeoPackage or other database types using `createSSURGO()` or the [SSURGOPortal R package]().

Assume you have a GeoPackage from SSURGOPortal (`"soilmu_a.gpkg"`), then you can read it with `sf::st_read()` or `terra::vect()`
...
- The `get_SDA_coecoclass()` function is the only known pre-written method that includes `cokey` in the dominant component aggregation.
- The DBF format used in shapefiles truncates column names. Consider alternate vector formats such as GeoPackage (`.gpkg` file) for full attribute support. 
- This approach avoids the need for SSURGOPortal or ESRI tools and is fully scriptable.
