...
Finally, we can write our result out to a spatial file, such as a GeoPackage, using `sf` or `terra`. We use the GeoPackage format because it is open and non-proprietary. Note that the DBF format used in ESRI shapefiles truncates column names. Use alternate vector formats like GeoPackage (`.gpkg` file) for full attribute support.
