...
.constructInterpQuery <- function(method, interp, areasymbols = NULL, mukeys = NULL, WHERE = NULL) {
  ...
  switch(agg_method$method,
         "DOMINANT COMPONENT" = .interpretation_aggregation(interp, WHERE, dominant = TRUE),
         "DOMINANT CONDITION" = .interpretation_by_condition(interp, WHERE, dominant = TRUE),
         "WEIGHTED AVERAGE" =   .interpretation_weighted_average(interp, WHERE),
         "NONE" =               .interpretation_aggregation(interp, WHERE)
  )
}
...
.interpretation_by_condition <- function(interp, where_clause, dominant = TRUE) {
  ...
  paste0(sapply(interp, function(x) sprintf("  (SELECT TOP 1 ROUND (AVG(interphr) OVER (PARTITION BY interphrc), 2)
   ...
   INNER JOIN cointerp ON c.cokey = cointerp.cokey AND mapunit.mukey = mu.mukey AND ruledepth = 0 AND mrulename LIKE '%s' GROUP BY interphrc, interphr
...
.interpretation_aggregation <- function(interp, where_clause, dominant = FALSE) {
  ...
  ifelse(dominant, "AND component.cokey = (SELECT TOP 1 c1.cokey FROM component AS c1
