...
#'
#' @details Soil properties supplied in \code{x} must be described, in order, via \code{vars} argument. The API does not use the names but column ordering must follow: sand, silt, clay, bulk density, volumetric water content at 33kPa (1/3 bar), and volumetric water content at 1500 kPa (15 bar).
#'
...
#'
#' @return a \code{data.frame} object with the following columns:
#' * `...`: pre-existing columns from \code{x}
#' * `theta_r`: residual volumetric water content (cm^3/cm^3)
#' * `theta_s`: saturated volumetric water content (cm^3/cm^3)
#' * `alpha`: related to the inverse of the air entry suction, log10-transformed values with units of cm
#' * `npar`: index of pore size distribution, log10-transformed values with units of 1/cm
#' * `ksat`: saturated hydraulic conductivity, log10-transformed values with units of cm/day
#'  * `.rosetta.model`: best-available model selection (-1 signifies that prediction was not possible due to missing values in \code{x})
#'  * `.rosetta.version`: ROSETTA algorithm version, selected via function argument \code{v}
#'
#' @references
...
#' ROSETTA Model API
#' 
#' A simple interface to the [ROSETTA model](https://www.ars.usda.gov/pacific-west-area/riverside-ca/agricultural-water-efficiency-and-salinity-research-unit/docs/model/rosetta-model/) for predicting hydraulic parameters from soil properties. The ROSETTA
#' API was developed by Dr. Todd Skaggs (USDA-ARS) and links to the work of
#' Zhang and Schaap, (2017). See the [ROSETTA API tutorial](http://ncss-tech.github.io/AQP/soilDB/ROSETTA-API.html) for additional examples.
#' 
#' Soil properties supplied in \code{x} must be described, in order, via
#' \code{vars} argument. The API does not use the names but column ordering
#' must follow: sand, silt, clay, bulk density, volumetric water content at
#' 33kPa (1/3 bar), and volumetric water content at 1500 kPa (15 bar).
#' 
#' The ROSETTA model relies on a minimum of 3 soil properties, with increasing
#' (expected) accuracy as additional properties are included: \itemize{ \item
#' required, \code{sand}, \code{silt}, \code{clay}: USDA soil texture separates
#' (percentages) that sum to 100\% \item optional, \verb{bulk density} (any
#' moisture basis): mass per volume after accounting for >2mm fragments, units
#' of gm/cm3 \item optional, \verb{volumetric water content at 33 kPa}: roughly
#' "field capacity" for most soils, units of cm^3/cm^3 \item optional,
#' \verb{volumetric water content at 1500 kPa}: roughly "permanent wilting
#' point" for most plants, units of cm^3/cm^3 }
#' 
#' Column names not specified in \code{vars} are retained in the output.
#' 
#' Three versions of the ROSETTA model are available, selected using \code{v =
#' 1}, \code{v = 2}, or \code{v = 3}. \itemize{ \item \strong{version 1}:
...
#' pedotransfer functions. Journal of Hydrology 251(3-4): 163-176. doi:
#' c("\\Sexpr[results=rd,stage=build]{tools:::Rd_expr_doi(\"#1\")}",
#' "10.1016/S0022-1694(01)00466-8")\Sexpr{tools:::Rd_expr_doi("10.1016/S0022-1694(01)00466-8")}.
#' \item \strong{version 2}: Schaap, M.G., A. Nemes, and M.T. van Genuchten.
#' 2004. Comparison of Models for Indirect Estimation of Water Retention and
#' Available Water in Surface Soils. Vadose Zone Journal 3(4): 1455-1463. doi:
#' c("\\Sexpr[results=rd,stage=build]{tools:::Rd_expr_doi(\"#1\")}",
#' "10.2136/vzj2004.1455")\Sexpr{tools:::Rd_expr_doi("10.2136/vzj2004.1455")}.
#' \item \strong{version 3}: Zhang, Y., and M.G. Schaap. 2017. Weighted
#' recalibration of the Rosetta pedotransfer model with improved estimates of
#' hydraulic parameter distributions and summary statistics (Rosetta3). Journal
#' of Hydrology 547: 39-53. doi:
#' c("\\Sexpr[results=rd,stage=build]{tools:::Rd_expr_doi(\"#1\")}",
#' "10.1016/j.jhydrol.2017.01.004")\Sexpr{tools:::Rd_expr_doi("10.1016/j.jhydrol.2017.01.004")}
#' }
#' 
#' @param x a \code{data.frame} of required soil properties, may contain other
#' columns, see details
#' @param vars character vector of column names in \code{x} containing relevant
#' soil property values, see details
#' @param v ROSETTA model version number: '1', '2', or '3', see details and
#' references.
#' @param chunkSize number of records per API call
#' @param conf configuration passed to \code{httr::POST()} such as
#' \code{verbose()}.
#' @return a \code{data.frame} object with the following columns: \itemize{
#' \item \code{...}: pre-existing columns from \code{x} \item \code{theta_r}:
#' residual volumetric water content (cm^3/cm^3) \item \code{theta_s}:
#' saturated volumetric water content (cm^3/cm^3) \item \code{alpha}: related
#' to the inverse of the air entry suction, log10-transformed values with units
#' of cm \item \code{npar}: index of pore size distribution, log10-transformed
#' values with units of 1/cm \item \code{ksat}: saturated hydraulic
#' conductivity, log10-transformed values with units of cm/day \item
#' \code{.rosetta.model}: best-available model selection (-1 signifies that
#' prediction was not possible due to missing values in \code{x}) \item
#' \code{.rosetta.version}: ROSETTA algorithm version, selected via function
#' argument \code{v} }
#' @note Input data should not contain columns names that will conflict with
#' the ROSETTA API results: \code{theta_r}, \code{theta_s}, \code{alpha},
#' \code{npar}, \code{ksat}.
#' @author D.E. Beaudette, Todd Skaggs (ARS), Richard Reid
#' @references Consider using the interactive version, with copy/paste
#' functionality at: \url{https://www.handbook60.org/rosetta}.
#' 
#' Rosetta Model Home Page:
#' \url{https://www.ars.usda.gov/pacific-west-area/riverside-ca/agricultural-water-efficiency-and-salinity-research-unit/docs/model/rosetta-model/}.
#' 
#' Python ROSETTA model: \url{http://www.u.arizona.edu/~ygzhang/download.html}.
#' 
#' Yonggen Zhang, Marcel G. Schaap. 2017. Weighted recalibration of the Rosetta
...
#' 39-53. c("\\Sexpr[results=rd,stage=build]{tools:::Rd_expr_doi(\"#1\")}",
#' "10.1016/j.jhydrol.2017.01.004")\Sexpr{tools:::Rd_expr_doi("10.1016/j.jhydrol.2017.01.004")}.
#' 
#' Kosugi, K. 1999. General model for unsaturated hydraulic conductivity for
#' soils with lognormal pore-size distribution. Soil Sci. Soc. Am. J.
#' 63:270-277.
#' 
...
#' 
#' Schaap, M.G., F.J. Leij, 1999, Improved prediction of unsaturated hydraulic
#' conductivity with the Mualem-van Genuchten, Submitted to Soil Sci. Soc. Am.
#' J.
#' 
...
#' "10.1016/S0022-1694(01)00466-8")\Sexpr{tools:::Rd_expr_doi("10.1016/S0022-1694(01)00466-8")}.
#' 
#' Schaap, M.G., A. Nemes, and M.T. van Genuchten. 2004. Comparison of Models
#' for Indirect Estimation of Water Retention and Available Water in Surface
#' Soils. Vadose Zone Journal 3(4): 1455-1463. doi:
#' c("\\Sexpr[results=rd,stage=build]{tools:::Rd_expr_doi(\"#1\")}",
#' "10.2136/vzj2004.1455")\Sexpr{tools:::Rd_expr_doi("10.2136/vzj2004.1455")}.
#' 
