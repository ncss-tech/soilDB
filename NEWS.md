# soilDB 2.8.9 (development)
 ...
 - `get_SDA_property()` `method` `"weighted average"` and `"min/max"` now use Common Table Expressions (CTEs) and therefore are compatible with local (SQLite or other) data sources 
