...
test_that("simplifyColorData: single color / moisture state / horizon", {
  ...
  expect_identical(res$d_r, res.rgb$r[1])
  expect_identical(res$d_g, res.rgb$g[1])
  expect_identical(res$d_b, res.rgb$b[1])

  # check parsing / conversion of moist color
  expect_identical(res$m_r, res.rgb$r[2])
  expect_identical(res$m_g, res.rgb$g[2])
  expect_identical(res$m_b, res.rgb$b[2])
...
test_that(".dominantColors: missing data", {
  ...
  expect_identical(res$d_hue[idx1], '2.5Y')
  expect_identical(res$d_value[idx1], 6)
  expect_identical(res$d_chroma[idx1], 1)

  # moist color dominant should be 10YR 5/6 (80%)
  idx2 <- which(res$phiid == "2889103")
  expect_identical(res$m_hue[idx2], '10YR')
  expect_identical(res$m_value[idx2], 5)
  expect_identical(res$m_chroma[idx2], 6)
