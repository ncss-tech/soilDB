...
test_that("artifactSieve puts artifacts without huartsize into 'unspecified' class", {
  ...
  expect_identical(res$class, 'art_unspecified')
...
test_that("artifactSieve assumptions are applied, results correct", {
  ...
  expect_identical(res$huartshp, 'irregular')
  expect_identical(res$huartco, 'cohesive')
...
test_that("artifactSieve safe fall-back from high to rv fragsize", {
  ...
  expect_identical(res$class, 'art_gr')
...
test_that("artifactSieve complex sample data from NASIS, single horizon", {
  ...
  # correct classes
  expect_identical(res$class, c('art_gr', 'art_cb', 'art_ch'))
...
test_that("simplifyArtifactData complex sample data from NASIS, single horizon", {
  ...
  expect_identical(res$art_fgr, c(0,0))
  expect_identical(res$art_gr, c(10,0))
  expect_identical(res$art_cb, c(5,0))
  expect_identical(res$art_ch, c(0,95))
  expect_identical(res$art_fl, c(0,0))
  expect_identical(res$art_st, c(0,0))
  
  # correct total 
  expect_identical(res$total_art_pct, c(15,95))
  
  # correct subtotal cohesive
  expect_identical(res$huartvol_cohesive, c(15, 0))  
  
  # correct subtotal penetrable
  expect_identical(res$huartvol_penetrable, c(0,95))  
  
  # correct subtotal noxious
  expect_identical(res$huartvol_noxious, c(0,0))
  
  # correct subtotal persistent
  expect_identical(res$huartvol_persistent, c(15,95))
...
test_that("simplifyArtifactData when missing fragment sizes, low/rv/high", {
  ...
  expect_identical(res$art_unspecified, res$total_art_pct)
...
test_that("simplifyArtifactData nullFragsAreZero works as expected", {
  ...
  expect_identical(as.logical(is.na(a)), c(FALSE, FALSE, TRUE, TRUE, FALSE, FALSE, FALSE, TRUE, TRUE, 
