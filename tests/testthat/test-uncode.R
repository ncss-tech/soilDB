test_that("uncode() works", {
  ...
  expect_identical(uncode(x)$texcl, c("cos", "s", "fs", "vfs", "lcos", "ls", "lfs", "lvfs", "cosl", "sl")) 
})

test_that("uncode() works w/ NASISDomainsAsFactor(TRUE)", {
  NASISDomainsAsFactor(TRUE)
  x <- data.frame(texcl = 1:10)
  expect_identical(uncode(x)$texcl, structure(1:10, .Label = c("cos", "s", "fs", "vfs", "lcos", "ls", 
...
test_that("code() works", {
  ...
  expect_identical(code(x)$texcl, 1:10) 
...
test_that("NASISChoiceList() works", {
  ...
  expect_identical(x, structure(c(3L, 12L, 5L), .Label = c("c", "cl", "cos", "cosl", 
  ...
  expect_identical(x, c('none', 'rare', 'occasional'))
  ...
  expect_identical(x, structure(5L, .Label = c("none", "very rare", "rare", "occasional", 
  ...
  expect_identical(x, structure(NA_integer_, levels = c("none", "very rare", "rare", 
