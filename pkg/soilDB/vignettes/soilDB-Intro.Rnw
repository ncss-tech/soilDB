...
% sweave / knitr config
\SweaveOpts{fig.path=figure/graphics-, cache.path=cache/graphics-, fig.align=center, dev=pdf, fig.width=5, fig.height=5, fig.show=hold, dpi=100, cache=TRUE, par=TRUE, warning=FALSE, message=FALSE, background=0.97;0.97;0.97}
...
\begin{figure}[h]
<<basic_plotting, dev=pdf, fig.width=12, fig.height=6, out.width=\linewidth, echo=TRUE, background=1;1;1>>=
...
\begin{figure}[h]
<<slicing-plot, dev=pdf, fig.width=10, fig.height=4, out.width=\linewidth, echo=TRUE, background=1;1;1>>=
...
\begin{figure}[h]
<<indexing_1, dev=pdf, fig.width=12, fig.height=6, out.width=\linewidth, echo=TRUE, background=1;1;1>>=
...
\begin{figure}[h]
<<indexing_2, dev=pdf, fig.width=6, fig.height=3, out.width=0.5\linewidth, echo=TRUE, background=1;1;1>>=
...
\begin{figure}[h]
<<indexing_3, dev=pdf, fig.width=10, fig.height=4, out.width=\linewidth, echo=TRUE, background=1;1;1>>=
...
\begin{figure}[ht]
<<profile_plot_annotation, dev=pdf, fig.width=12, fig.height=6, out.width=\linewidth, echo=TRUE, background=1;1;1>>=
...
\begin{figure}[h]
<<profile_aggregation, dev=pdf, fig.width=8, fig.height=4, out.width=0.75\linewidth, echo=TRUE, background=1;1;1>>=
...
\begin{figure}[h]
<<dend_with_profiles, dev=pdf, fig.width=12, fig.height=6, out.width=\linewidth, echo=TRUE, dependson=gopheridge-demo, background=1;1;1>>=
...
\begin{figure}[h]
<<double_dend, dev=pdf, fig.width=12, fig.height=6, out.width=\linewidth, echo=TRUE, background=1;1;1>>=
## 
...
# better plotting of dendrograms
if(require(ape)) {
	...
	fig.x.max <- 2
	arrow.left <- fig.x.max * 0.42 # hand-tune
	arrow.right <- fig.x.max * 0.62 # hand-tune
	arrow.length <- 0.1
	
	# plot hz-level dissimilarities on left-hand side
	plot(phylo.hz, cex=0.65, font=1,
	direction='rightwards', no.margin=TRUE, label.offset=0.015, x.lim=c(0, fig.x.max))
	...
	## TODO: how can we scale this dendrogram to match the previous one?
	# plot site+hz-level dissimilarities on right-hand side
	par(new=TRUE) # keep the existing plot, and add to it
	plot(phylo.site.hz, cex=0.65, font=1,
	direction='leftwards',no.margin=TRUE, label.offset=0.015, x.lim=c(0, fig.x.max))
	...
	# annotate with axis to assist with placement
	s <- seq(from=0, to=fig.x.max, by=0.1)
	abline(v=s, lty=3, col='grey')
	text(x=s, y=max(p.left$yy)+1.5, labels=s, cex=0.65)
  ...
  y1=p.right$yy[left.right.converstion.order], col='RoyalBlue')
	...
	# plot helper arrows
	arrows(x0=arrow.left, y0=p.left$yy[left.new_order], x1=arrow.left-arrow.length, y1=p.left$yy[left.new_order], col='RoyalBlue', code=2, length=0.1)
	arrows(x0=arrow.right, y0=p.right$yy[right.new_order], x1=arrow.right+arrow.length, y1=p.right$yy[right.new_order], col='RoyalBlue', code=2, length=0.1)
	
	# annotate with type of dissimilarity calculation
	mtext('Horizon Data', side=2, line=-1, font=2)
	mtext('Horizon + Site Data', side=4, line=-1.5, font=2)
	
	# legend
	legend('bottomright', legend=c('Paralithic Contact'), pch=15, col='orange', bty='n')
	
	# check: profiles missing lithic contact:
	profile_id(loafercreek[which( ! loafercreek$paralithic.contact), ])
}
@
\caption{\label{double_dend} hz-level vs hz+site-level dissimilarity calculation.}
\end{figure}
